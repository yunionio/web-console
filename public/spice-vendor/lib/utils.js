...
wdi.Utils = {
  generateWebSocketUrl: function (protocol, host, port, destHost, destPort, type, destInfoToken) {
    ...
    var hadPort = function (value) {
      var reg = /^.+:\d+$/
      return reg.test(value)
    }
    var parentQuery = window.parent.getQuery()
    var _port = ''
    var _host = parentQuery.api_server.slice(parentQuery.api_server.indexOf('//') + 2) // 去掉双划线
    if (hadPort(_host)) {
      _port = _host.slice(_host.indexOf(':') + 1) // 去掉:
      _host = _host.slice(0, _host.indexOf(':'))
    } else {
      _port = parentQuery.api_server.indexOf('https') === 0 ? 443 : 80
    }
    var _scheme = 'ws'
    if (parentQuery.api_server.indexOf('https') === 0) {
      _scheme = 'wss'
    }
    url = _scheme + '://' + _host + ':' + _port + '/websockify/?access_token=' + parentQuery.access_token
    console.log(url, 'url')
    return url
  }
}
