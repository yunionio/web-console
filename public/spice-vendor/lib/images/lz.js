wdi.LZSS = {
  ...
  lz_rgb32_decompress: function (in_buf, at, out_buf, type, default_alpha, palette, opaque) {
    ...
    var is_rgba = type === this.LZ_IMAGE_TYPE_RGBA
        ...
        if (ref === (op - 1)) { // plt4/1 what?
          ...
          for (; len; --len) {
            ...
            if (is_rgba) {
              ...
              for (var i = 0; i < 4; i++) { out_buf[(op_4) + i] = out_buf[(b * 4) + i] }
            }
            op++
          }
        } else {
          for (; len; --len) {
            ...
            if (is_rgba) {
              ...
              for (i = 0; i < 4; i++) { out_buf[(op_4) + i] = out_buf[(ref * 4) + i] }
            }
            op++; ref++
          }
        }
        ...
        if (is_rgba) {
          ...
          if (type === this.LZ_IMAGE_TYPE_PLT1_LE) {
            ...
            for (var i = 0; i < 8; i++) {
              ...
              if (default_alpha) { out_buf[(op_4) + 3] = 255 }
              if (i < 7) { op++ } op_4 = op * 4
            }
