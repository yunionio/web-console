        ...
        <a-dropdown :trigger="['click']">
          ...
          <a-menu slot="overlay">
            <template v-for="(item, idx) in dropdownOps">
              <a-menu-item :key="item" @click="dropdownClick({key: item})">{{ 'Ctrl+Alt+' + item }}</a-menu-item>
              <a-menu-divider v-if="item === 'F1'" :key="idx" />
            </template>
            <a-menu-divider />
            <a-menu-item key="custom" @click="openCustomKeyModal">自定义组合键</a-menu-item>
          </a-menu>
        </a-dropdown>
